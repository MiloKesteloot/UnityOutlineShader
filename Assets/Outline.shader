    ...
    SubShader
    {
        ...
        Pass
        {
			...
			float4 Frag(Varyings i) : SV_Target
			{
                ...
                // 1. Sample colors at the same points
                ...
                // 2. Convert colors to luminance (grayscale)
                float luminance0 = color0.r * 0.3 + color0.g * 0.59 + color0.b * 0.11;
                float luminance1 = color1.r * 0.3 + color1.g * 0.59 + color1.b * 0.11;
                float luminance2 = color2.r * 0.3 + color2.g * 0.59 + color2.b * 0.11;
                float luminance3 = color3.r * 0.3 + color3.g * 0.59 + color3.b * 0.11;

                // 3. Apply Roberts cross operator to luminance
                float luminanceFiniteDifference0 = luminance1 - luminance0;
                float luminanceFiniteDifference1 = luminance3 - luminance2;
                float edgeLuminance = sqrt(
                    pow(luminanceFiniteDifference0, 2) + 
                    pow(luminanceFiniteDifference1, 2)
                );

                // 4. Threshold luminance edge
